import 'package:derbymatch/core/theme/pallete.dart';
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';

import '../../../../../core/values/values.dart';
import '../../../../core/common/controllers/CommCodeController.dart';
import '../../controllers/PlayerFilterController.dart';

class GenderChoiceChipFilter extends ConsumerWidget {
  const GenderChoiceChipFilter({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final CommCodeController = ref.read(commCodeControllerProvider.notifier);
    List genderList = CommCodeController.getCommCodesByType('gender');
    final playerFilter = ref.watch(playerFilterProvider.notifier);
    final selectedGenders = ref.watch(playerFilterProvider).gender;

    return Wrap(
      alignment: WrapAlignment.start, // 왼쪽 정렬
      spacing: AppSpaceSize.smallSize,
      children: List<Widget>.generate(genderList.length, (int index) {
        bool isSelected =
            selectedGenders.contains(genderList[index]['comm_cd']);
        return ChoiceChip(
          selectedColor: Pallete.primaryColor,
          side: isSelected ? null : BorderSide(color: Pallete.primaryColor),
          // 선택되지 않았을 때의 테두리 색상
          checkmarkColor: Pallete.whiteColor,
          // 체크 아이콘 색상
          label: Text(
            genderList[index]['comm_cd_nm'],
            style: isSelected
                ? AppTextStyles.cautionTextStyle
                    .copyWith(color: Pallete.whiteColor)
                : AppTextStyles.cautionTextStyle
                    .copyWith(color: Pallete.primaryColor),
          ),
          selected: isSelected,
          onSelected: (bool selected) {
            playerFilter.updateGender(genderList[index]['comm_cd']);
          },
        );
      }),
    );
  }
}
