...
class DivisionChoiceChipFilter extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final teamFilter = ref.watch(teamFilterProvider.notifier);
    final selectedDivisions = ref.watch(teamFilterProvider).division;

    return Wrap(
      alignment: WrapAlignment.start, // 왼쪽 정렬
      spacing: AppSpaceSize.smallSize,
      children: List<Widget>.generate(divisionList.length, (int index) {
        bool isSelected =
        ...
        return ChoiceChip(
          ...
          onSelected: (bool selected) {
            teamFilter.updateDivision(divisionList[index].comm_cd);
          },
        );
      }),
    );
